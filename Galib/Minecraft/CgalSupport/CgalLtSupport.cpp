...
// ---------------------------------------------------------------------------
namespace {
    using LtVertexIndex = galib::minecraft::cgal_support::SurfaceMeshType::Vertex_index;
    using LtPoint = galib::minecraft::cgal_support::LtPoint3;

    // 四个点是否共面（坐标此时是 grid 单位，尺度 <= 数百，1e-9 的绝对容差足够）
    bool isPlanarQuad(const LtPoint &kA, const LtPoint &kB, const LtPoint &kC, const LtPoint &kD) {
        const double abx = kB.x() - kA.x(), aby = kB.y() - kA.y(), abz = kB.z() - kA.z();
        const double acx = kC.x() - kA.x(), acy = kC.y() - kA.y(), acz = kC.z() - kA.z();
        const double nx = aby * acz - abz * acy;
        const double ny = abz * acx - abx * acz;
        const double nz = abx * acy - aby * acx;
        const double length = std::sqrt(nx * nx + ny * ny + nz * nz);
        if (length < 1e-12) { return true; }    // 退化面，交给 CGAL 自行拒绝

        const double adx = kD.x() - kA.x(), ady = kD.y() - kA.y(), adz = kD.z() - kA.z();
    ...
    bool isSamePoint(const LtPoint &kA, const LtPoint &kB) {
        return std::fabs(kA.x() - kB.x()) < 1e-9 &&
               std::fabs(kA.y() - kB.y()) < 1e-9 &&
               std::fabs(kA.z() - kB.z()) < 1e-9;
    }

    // kIndices/kPoints 必须按同一环绕顺序给出（该顺序决定面的法线方向）
    void addQuadFace(
        galib::minecraft::cgal_support::SurfaceMeshType &mesh,
        const std::array<LtVertexIndex, 4> &kIndices,
        const std::array<LtPoint, 4> &kPoints,
        const bool kFlipped
    ) {
        ...
        for (GALIB_STD size_t i = 0; i < 4; ++i) {
            if (count > 0 && isSamePoint(points[count - 1], kPoints[i])) { continue; }
            indices[count] = kIndices[i];
            points[count] = kPoints[i];
            ++count;
        }
        while (count > 1 && isSamePoint(points[0], points[count - 1])) { --count; }
        if (count < 3) { return; }      // 退化成一个点或一条线，无面可加
...
void GALIB minecraft::cgal_support::createMeshFromTileEntity(
    ...
    const LtPoint p_eun = convertToCGALPoint(kTileEntity.getVertices(AngleID::EUN, kApplyOffset));
    const LtPoint p_eus = convertToCGALPoint(kTileEntity.getVertices(AngleID::EUS, kApplyOffset));
    const LtPoint p_edn = convertToCGALPoint(kTileEntity.getVertices(AngleID::EDN, kApplyOffset));
    const LtPoint p_eds = convertToCGALPoint(kTileEntity.getVertices(AngleID::EDS, kApplyOffset));
    const LtPoint p_wun = convertToCGALPoint(kTileEntity.getVertices(AngleID::WUN, kApplyOffset));
    const LtPoint p_wus = convertToCGALPoint(kTileEntity.getVertices(AngleID::WUS, kApplyOffset));
    const LtPoint p_wdn = convertToCGALPoint(kTileEntity.getVertices(AngleID::WDN, kApplyOffset));
    const LtPoint p_wds = convertToCGALPoint(kTileEntity.getVertices(AngleID::WDS, kApplyOffset));
    ...
    addQuadFace(mesh, {eds, edn, eun, eus}, {p_eds, p_edn, p_eun, p_eus}, east_flipped);
    addQuadFace(mesh, {wdn, wds, wus, wun}, {p_wdn, p_wds, p_wus, p_wun}, west_flipped);
    addQuadFace(mesh, {wds, eds, eus, wus}, {p_wds, p_eds, p_eus, p_wus}, south_flipped);
    addQuadFace(mesh, {edn, wdn, wun, eun}, {p_edn, p_wdn, p_wun, p_eun}, north_flipped);
    addQuadFace(mesh, {wus, eus, eun, wun}, {p_wus, p_eus, p_eun, p_wun}, up_flipped);
    addQuadFace(mesh, {wdn, edn, eds, wds}, {p_wdn, p_edn, p_eds, p_wds}, down_flipped);
}
...
namespace {
    ...
    ClipVec3 clipLerp(const ClipVec3 &kA, const ClipVec3 &kB, const double kT) {
        return {kA.x + (kB.x - kA.x) * kT, kA.y + (kB.y - kA.y) * kT, kA.z + (kB.z - kA.z) * kT};
    }
    ...
    ClipPolyhedron clipPolyhedronByPlane(const ClipPolyhedron &kPolyhedron, const ClipVec3 &kN, const double kD) {
        ...
        for (const ClipPolygon &face : kPolyhedron) {
            ...
            for (GALIB_STD size_t i = 0; i < count; ++i) {
                ...
                if (current_inside != next_inside) {
                    ...
                    if (GALIB_STD fabs(denominator) < kClipEpsilon) { continue; }
                    const ClipVec3 cross_point = clipLerp(current, next, current_distance / denominator);
            ...
            if (simplified.size() >= 2 && clipNearlyEqual(simplified.front(), simplified.back())) {
                simplified.pop_back();
            }
            if (simplified.size() >= 3) { result.push_back(simplified); }
        }
        ...
        for (const ClipVec3 &point : cut_points) {
            ...
            for (const ClipVec3 &kept : unique_points) {
                if (clipNearlyEqual(point, kept)) { duplicated = true; break; }
            }
            if (!duplicated) { unique_points.push_back(point); }
        }
        ...
        if (unique_points.size() >= 3) {
            ...
            const double inverse_count = 1.0 / static_cast<double>(unique_points.size());
            ...
            ClipVec3 u = (GALIB_STD fabs(kN.x) <= GALIB_STD fabs(kN.y) && GALIB_STD fabs(kN.x) <= GALIB_STD fabs(kN.z))
                         ? ClipVec3{1.0, 0.0, 0.0}
                         : ((GALIB_STD fabs(kN.y) <= GALIB_STD fabs(kN.z)) ? ClipVec3{0.0, 1.0, 0.0}
            ...
            if (u_length < kClipEpsilon) { return result; }
            ...
            GALIB_STD sort(unique_points.begin(), unique_points.end(),
    ...
    ClipVec3 clipToVec3(const GALIB minecraft::littletiles::LittleTilesCoord &kCoord) {
    ...
    void appendQuadFace(ClipPolyhedron &desc_polyhedron, ClipPolygon kPoints, const bool kFlipped) {
        ClipPolygon polygon;
        for (const ClipVec3 &point : kPoints) {
            if (polygon.empty() || !clipNearlyEqual(polygon.back(), point)) { polygon.push_back(point); }
        }
        while (polygon.size() > 1 && clipNearlyEqual(polygon.front(), polygon.back())) { polygon.pop_back(); }
...
bool GALIB minecraft::cgal_support::clipTileEntityToBox(
    ...
    // 8 个角点（grid 单位）
    const ClipVec3 eun = clipToVec3(kTileEntity.getVertices(AngleID::EUN, kApplyOffset));
    const ClipVec3 eus = clipToVec3(kTileEntity.getVertices(AngleID::EUS, kApplyOffset));
    const ClipVec3 edn = clipToVec3(kTileEntity.getVertices(AngleID::EDN, kApplyOffset));
    const ClipVec3 eds = clipToVec3(kTileEntity.getVertices(AngleID::EDS, kApplyOffset));
    const ClipVec3 wun = clipToVec3(kTileEntity.getVertices(AngleID::WUN, kApplyOffset));
    const ClipVec3 wus = clipToVec3(kTileEntity.getVertices(AngleID::WUS, kApplyOffset));
    const ClipVec3 wdn = clipToVec3(kTileEntity.getVertices(AngleID::WDN, kApplyOffset));
    const ClipVec3 wds = clipToVec3(kTileEntity.getVertices(AngleID::WDS, kApplyOffset));
    ...
    const ClipVec3 box_a = clipToVec3(kTileEntity.getVertices(AngleID::WDN, false));
    const ClipVec3 box_b = clipToVec3(kTileEntity.getVertices(AngleID::EUS, false));
    const ClipVec3 box_min{GALIB_STD min(box_a.x, box_b.x), GALIB_STD min(box_a.y, box_b.y), GALIB_STD min(box_a.z, box_b.z)};
    const ClipVec3 box_max{GALIB_STD max(box_a.x, box_b.x), GALIB_STD max(box_a.y, box_b.y), GALIB_STD max(box_a.z, box_b.z)};
    ...
    if (polyhedron.empty()) { return false; }

    SurfaceMeshType &mesh = desc_mesh.getMesh();
    GALIB_STD map<GALIB_STD array<long long, 3>, SurfaceMeshType::Vertex_index> welded_vertices;
    ...
    for (const ClipPolygon &face : polyhedron) {
        ...
        for (const ClipVec3 &point : face) {
            ...
            if (found == welded_vertices.end()) {
                found = welded_vertices.emplace(key, mesh.add_vertex(LtPoint(point.x, point.y, point.z))).first;
            }
...
void (GALIB minecraft::cgal_support::cleanupMesh)(LtSurfaceMesh& mesh) {
    ...
    if (CGAL::is_triangle_mesh(surface_mesh)) {
        ...
        for (SurfaceMeshType::Face_index face : surface_mesh.faces()) {
            ...
            for (SurfaceMeshType::Vertex_index vertex : vertices_around_face(surface_mesh.halfedge(face), surface_mesh)) {
                face_vertices.push_back(vertex);
            }

            bool is_degenerated = face_vertices.size() < 3;
            for (GALIB_STD size_t i = 0; !is_degenerated && i < face_vertices.size(); ++i) {
                for (GALIB_STD size_t j = i + 1; j < face_vertices.size(); ++j) {
                    if (face_vertices[i] == face_vertices[j]) { is_degenerated = true; break; }
                }
            }
            if (is_degenerated) { degenerated_faces.push_back(face); }
        }
