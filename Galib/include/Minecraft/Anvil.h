 ...
 public:
  ...
  bool HasCache(const Coord2dType& kInteger2dCoord) const {
    ...
    return cache_[kInteger2dCoord.x].get() &&
           (*cache_[kInteger2dCoord.x].get())[kInteger2dCoord.z].get();
  }
 ...
 private:
  struct ChunkConstIterator {
    ...
    ByteArray::const_iterator
        valid_begin;  // Valid data start iterator (NBT compressed data)
    ByteArray::const_iterator
        valid_end;  // Valid data end iterator (NBT compressed data)
    ...
    ByteIndex chunk_offset{
        0};  // Chunk offset (original value, in units of 4096 bytes)
    ByteIndex chunk_offset_data_index{
        0};  // Byte position of the block (in bytes)
    ByteIndex chunk_size{
        0};  // Chunk size (in bytes, equivalent to the chunk's occupied sector size)
