...
using BlockCoordinate = galib::coord::Coordinate3D<std::int32_t>;

// Region coordinates (r.x,y.mca): This type is a signed 32-bit integer, specifically
// int32_t from the standard library.
using RegionCoordinate = galib::coord::Coordinate2D<std::int32_t>;
...
using RegionChunkCoordinate = galib::coord::Coordinate2D<std::int32_t>;

// Chunk coordinates: These coordinates are in the world reference system.
// This type is a signed 32-bit integer, specifically int32_t from the standard library.
using ChunkCoordinate = galib::coord::Coordinate2D<std::int32_t>;
...
// This type is a signed 32-bit integer, specifically int32_t from the standard library.
using SingleChunkCoordinate = galib::coord::Coordinate2D<std::int32_t>;
...
// coordinate transformation
// Please refer to Chapter 1, Section 3 of the document "Minecraft and LittleTiles Export Obj Basic Technology Research."
template <typename Coord2dType, typename NumericType = typename galib::coord::
                                    coord_type_traits<Coord2dType>::NumericType>
Coord2dType CoordSwap2D(const Coord2dType& kOreCoord, const NumericType& base) {
  return {CoordSwap<NumericType>(kOreCoord.x, base),
          CoordSwap<NumericType>(kOreCoord.z, base)};
};

inline bool IsValidCheckForRegionChunkCoord(
...
RegionChunkCoordinate ChunkCoordToRegionChunkCoord(
